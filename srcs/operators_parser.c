...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/15 20:13:59 by aouichou          #+#    #+#             */
/*   Updated: 2023/02/15 20:34:54 by aouichou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int	count_words(char *line, char *set, int i[2])
{
	...
	while (line[i[0]])
	{
		...
		{
			i[1]++;
			while ((!ft_strchr(set, line[i[0]]) || prth[0]) && line[i[0]])
			{
				if (!prth[1] && (line[i[0]] == '(' || line[i[0]] == ')'))
					prth[1] = line[i[0]];
				prth[0] = (prth[0] + (line[i[0]] == prth[1])) % 2;
				prth[1] *= prth[0] != 0;
				i[0]++;
			}
			if (prth[0])
